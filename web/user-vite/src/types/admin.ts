...
// ===== Exit Node Sets (出口节点集合) =====
export interface ExitNodeSetMember {
  id: number;
  set_id: number;
  agent_host_id: number;
  weight: number;
...
export interface ExitNodeSetDetail {
  set: ExitNodeSet;
  members: ExitNodeSetMember[];
  host_name: Record<string, string>;
}

export interface CreateExitNodeSetRequest {
  name: string;
  description?: string;
  tags?: string[];
  strategy?: string;
  members?: { agent_host_id: number; weight?: number }[];
}

export interface UpdateExitNodeSetRequest {
  name?: string;
  description?: string;
  tags?: string[];
  strategy?: string;
...
export interface AddExitNodeSetMemberRequest {
  agent_host_id: number;
  weight?: number;
  enabled?: boolean;
}
...
export interface RoutingPolicy {
  ...
  core_type: string;
  priority: number;
  match_type: "geosite" | "domain" | "ip_cidr";
  match_value: string;
  action: string;
...
export type UpdateRoutingPolicyRequest = Partial<CreateRoutingPolicyRequest>;
