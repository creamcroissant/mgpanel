...
/**
 * 画布D —— Mesh 组网视图：
 * 节点 = mesh peer（WG 地址 + 在线态 + 探测计数），边 = 全互联 wgmesh0 隧道。
 * 数据源：拓扑快照 agents + /admin/mesh/network/default peers 合并。
 */
export interface MeshPeerLite {
...
export function assembleCanvasD(snapshot: TopologySnapshot, peers: MeshPeerLite[]): TopologyGraph {
  const nodes: TopoNode[] = [];
  const edges: TopoEdge[] = [];
  const agentById = new Map(snapshot.agents.map((a) => [a.id, a]));
  const onlineIds = new Set<number>();

  for (const mp of peers) {
    const a = agentById.get(mp.agent_host_id);
    if (mp.online) onlineIds.add(mp.agent_host_id);
    nodes.push(
      node("agent", mp.agent_host_id, {
        label: a?.name ?? `agent-${mp.agent_host_id}`,
        host: a?.host,
        online: a?.online ?? false,
        cores: a?.cores,
        wgIp: mp.wg_ip,
        probes: mp.total_probes,
      })
    );
  }

  // 全互联隧道边（WireGuard mesh 为全互联模型）；双端在线实线、否则虚线
  for (let i = 0; i < peers.length; i++) {
    for (let j = i + 1; j < peers.length; j++) {
      const up = onlineIds.has(peers[i].agent_host_id) && onlineIds.has(peers[j].agent_host_id);
      edges.push({
        id: `meshpair-${peers[i].agent_host_id}-${peers[j].agent_host_id}`,
        source: `${PREFIX.agent}-${peers[i].agent_host_id}`,
        target: `${PREFIX.agent}-${peers[j].agent_host_id}`,
        kind: "relay" satisfies EdgeKind,
        label: undefined,
        dimmed: !up,
      });
