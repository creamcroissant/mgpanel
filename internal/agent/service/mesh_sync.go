...
func (a *Agent) syncMeshConfig(ctx context.Context, mgr *mesh.Manager) {
	if mgr == nil {
		return
	}

	// Step 1: Check local config
	localCfg, err := mgr.ReadLocalConfig()
	if err != nil {
	...
	if localCfg == nil {
		...
		if err := mgr.WriteLocalConfig(*localCfg); err != nil {
		...
		if err := mgr.Start(ctx); err != nil {
			...
			if rmErr := mgr.RemoveLocalConfig(); rmErr != nil {
	...
	if _, err := mgr.DumpPeers(ctx); err != nil {
	...
	if len(peersResp.Peers) > 0 {
		...
		if err := mgr.SyncPeers(ctx, peerCfgs); err != nil {
...
func (a *Agent) cidrPrefixLen() int {
	...
	_, cidrNet, err := net.ParseCIDR(mgr.NetworkCIDR())
	if err != nil {
		slog.Warn("mesh sync: invalid NetworkCIDR, falling back to /24",
			"cidr", mgr.NetworkCIDR(),
		)
		return 24
	}
