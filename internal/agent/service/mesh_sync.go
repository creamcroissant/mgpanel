...
func (a *Agent) syncMeshConfig(ctx context.Context) {
	...
	if localCfg == nil {
		...
		wgConfig := a.meshManager.BuildWGConfig(*localCfg)
		if err := a.meshManager.ApplyWGConfig(ctx, wgConfig); err != nil {
			slog.Warn("mesh sync: apply wg config failed, rolling back local config", "error", err)
			// Remove local config so next sync retries auto-join
			if rmErr := a.meshManager.RemoveLocalConfig(); rmErr != nil {
				slog.Warn("mesh sync: failed to remove local config after apply failure", "error", rmErr)
			}
			return
		}
	...
	// Step 3: Fetch all peers via gRPC and sync
