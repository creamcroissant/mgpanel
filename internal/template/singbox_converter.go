...
func buildSingBoxInbound(inbound UnifiedInbound) map[string]any {
	...
	if inbound.TLS != nil && inbound.TLS.Enabled {
		...
		if inbound.TLS.Reality != nil && inbound.TLS.Reality.Enabled {
			...
			// sing-box 服务端 reality: server_name 是客户端字段，tls 层才接受 server_name。
			// 取数组首个作为 tls.server_name，reality 内不写 server_name。
			if len(inbound.TLS.Reality.ServerNames) > 0 && inbound.TLS.ServerName == "" {
				tls["server_name"] = inbound.TLS.Reality.ServerNames[0]
			}
			// fingerprint 是客户端 TLS 指纹参数，服务端 inbound 不识别此字段，仅用于订阅渲染
