...
if [ "$SKIP_SYSTEMD" != "1" ]; then
    if is_systemd_available; then
        ...
        # crashed/stopped 可能是误报（旧进程仍在跑但 OpenRC 状态脱节），
        # 先按 pidfile 确认进程真死再判 FAIL，避免“进程活着却报失败”。
        if run_privileged "$OPENRC_SERVICE_CMD" mgpanel-agent status >/dev/null 2>&1; then
            echo "[OK] mgpanel-agent service: active (openrc)"
        elif [ -f /run/mgpanel-agent.pid ] && kill -0 "$(cat /run/mgpanel-agent.pid 2>/dev/null | tr -d ' \t\r\n')" >/dev/null 2>&1; then
            echo "[OK] mgpanel-agent service: process running (openrc status lagging)"
        else
